...
const char * const BITCOIN_CONF_FILENAME = "litecoingreen.conf";
...
#else
    const char* pszModule = "litecoingreen";
#endif
    if (pex)
        return strprintf(
...
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "Litecoingreen";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/Litecoingreen";
#else
    // Unix
    return pathRet / ".litecoingreen";
#endif
#endif
}
